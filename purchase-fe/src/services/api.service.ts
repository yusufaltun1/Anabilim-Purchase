...
export async function registerCompany(data: CompanyRegistrationData): Promise<any> {
  const response = await fetch('https://api.talentai.com.tr/api/v1/auth/register', {
    method: 'POST',
    headers: {
...
export async function checkJobPositionCredits(): Promise<CreditsInfo> {
  const token = authService.getToken();
  const response = await fetch('https://api.talentai.com.tr/api/credits/check-job-position', {
    headers: {
...
export async function getCreditPackages(): Promise<CreditPackage[]> {
  const token = authService.getToken();
  const response = await fetch('https://api.talentai.com.tr/api/credits/packages', {
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
    },
...
export async function purchaseCredits(request: PurchaseRequest): Promise<PurchaseResponse> {
  const token = authService.getToken();
  const response = await fetch('https://api.talentai.com.tr/api/credits/purchase', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
...
export async function fetchAllCandidates(): Promise<any[]> {
  const token = authService.getToken();
  const response = await fetch('https://api.talentai.com.tr/api/recruitment/candidates/all', {
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
    },
