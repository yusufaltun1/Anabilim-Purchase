...
import { WorkflowStep, CreateWorkflowRequest } from '../types/workflow';
...
export const WorkflowCreate = () => {
  ...
  const validateForm = (): boolean => {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      if (!step.stepName.trim()) {
        setError(`${i + 1}. adım için adım adı gereklidir`);
        return false;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate('/workflows', { 
        state: { message: 'Workflow başarıyla oluşturuldu!' }
      });
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Workflow oluşturulurken hata oluştu');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCancel = () => {
    if (formData.name || formData.description || steps.some(s => s.roleName)) {
  ...
  // Rol seçim alanı için render fonksiyonu
  const renderRoleSelect = (stepIndex: number) => {
    if (loadingRoles) {
      return (
    ...
    return (
      <select
        value={steps[stepIndex].roleName}
        onChange={(e) => handleStepChange(stepIndex, 'roleName', e.target.value)}
      ...
      <div className="max-w-4xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div className="px-4 py-6 sm:px-0">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Yeni Onay Süreci</h1>
              <p className="mt-2 text-gray-600">
                Yeni bir onay süreci oluşturun ve adımlarını tanımlayın
              </p>
            </div>
