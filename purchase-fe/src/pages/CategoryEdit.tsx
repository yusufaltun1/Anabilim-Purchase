...
export const CategoryEdit = () => {
  ...
  const loadCategoryData = async () => {
    try {
      ...
      // Önce üst kategorileri yükle
      const parentResponse = await categoryService.getActiveCategories();
      console.log('Parent categories response:', parentResponse);
      
      if (parentResponse.success) {
        const categories = Array.isArray(parentResponse.data) 
          ? parentResponse.data as Category[] 
          : [parentResponse.data as Category];
        
        // Mevcut kategoriyi üst kategori listesinden çıkar
        const filteredCategories = categories.filter(c => c.id !== parseInt(id!));
        console.log('Available parent categories:', filteredCategories);
        setParentCategories(filteredCategories);
      }

      // Sonra kategori detaylarını yükle
      ...
      if (categoryResponse.success && categoryResponse.data) {
        ...
        console.log('Setting form data:', category);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await categoryService.updateCategory(parseInt(id!), formData);
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  ...
                  <p className="mt-1 text-sm text-green-700">
                    Kategori detay sayfasına yönlendiriliyorsunuz...
                  </p>
                </div>
              </div>
            </div>
