  ...
  const loadRequests = async () => {
    try {
      ...
      if (response.success) {
        ...
        setRequests(Array.isArray(requestsData) ? requestsData : (requestsData ? [requestsData] : []));
      } else {
        ...
        <div className="px-4 py-6 sm:px-0">
          ...
          <div className="mb-6">
            <div className="flex space-x-4">
              ...
              <button onClick={() => setFilter('pending')} className={`px-4 py-2 rounded-md text-sm font-medium ${filter === 'pending' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-500 hover:text-gray-700'}`}>
                Onay Bekleyenler
              </button>
            </div>
          </div>
          ...
          {error && (
            <div className="mb-6 bg-red-50 border border-red-200 rounded-md p-4">
              <div className="flex"><div className="flex-shrink-0"><svg className="h-5 w-5 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div><div className="ml-3"><h3 className="text-sm font-medium text-red-800">{error}</h3></div></div>
            </div>
          )}
          ...
          <div className="bg-white shadow overflow-hidden sm:rounded-md">
            {loading ? (
              ...
              <div className="text-center py-12"><p className="text-gray-500">{filter === 'my-requests' ? 'Henüz talep oluşturmadınız' : 'Gösterilecek talep bulunmuyor'}</p></div>
            ) : (
                      ...
                      <div className="px-4 py-4 sm:px-6">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center">
                            ...
                            <span className={`ml-2 px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusBadgeColor(request.status)}`}>{getStatusText(request.status)}</span>
                          </div>
                        ...
                        <div className="mt-2 sm:flex sm:justify-between">
                          <div className="sm:flex">
                            <p className="flex items-center text-sm text-gray-500">{request.requester.firstName} {request.requester.lastName}</p>
                            {getCurrentApprover(request) && <p className="mt-2 flex items-center text-sm text-gray-500 sm:mt-0 sm:ml-6">Onaylayacak: {getCurrentApprover(request)?.firstName} {getCurrentApprover(request)?.lastName}</p>}
                          </div>
                          <div className="mt-2 flex items-center text-sm text-gray-500 sm:mt-0"><p>Oluşturulma: {new Date(request.createdAt).toLocaleDateString('tr-TR')}</p></div>
                        </div>
                      </div>
                    </div>
                  </li>
