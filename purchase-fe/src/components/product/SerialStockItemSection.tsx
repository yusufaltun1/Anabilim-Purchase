    ...
    <div className="mt-6 bg-white shadow overflow-hidden sm:rounded-lg">
      <div className="px-4 py-5 sm:px-6">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Her cihaz ayrı kutuda — kutuya tıklayınca zimmet, hareketler ve geçmiş görünür
        </p>
      </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {stockItems.map((item) => {
              ...
              const title =
                [item.serialNumber, item.assetLabel].filter(Boolean).join(' · ') || `Cihaz #${item.id}`;

              return (
                <div
                  key={item.id}
                  className={`rounded-xl border bg-white shadow-sm transition-shadow ${
                    isExpanded ? 'border-indigo-300 ring-2 ring-indigo-100' : 'border-gray-200 hover:border-gray-300'
                  }`}
                >
                  <button
                    type="button"
                    onClick={() => toggleExpanded(itemId)}
                    className="w-full text-left p-4 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 rounded-xl"
                  >
                    ...
                    <p className="mt-3 text-xs text-indigo-600 font-medium">
                      {isExpanded ? 'Detayı gizle ↑' : 'Detay, zimmet ve hareketler →'}
                    </p>
                  </button>
